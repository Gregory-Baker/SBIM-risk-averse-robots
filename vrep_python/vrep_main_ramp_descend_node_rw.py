...
def reweight(weights,
    ...
    weights[i,j] = min(20, weights[i,j]*weight_multiplier)
    ...
    for x_delta in range(-1,2):
        for y_delta in range(-1,2):
            if x_delta != 0 or y_delta != 0:
                x_neigh = i + x_delta
                y_neigh = j + y_delta
                weights[x_neigh, y_neigh] = min(20, weights[x_neigh, y_neigh]*(weight_multiplier**0.5))
...
while trials < 50:
    ...
    if outcome == 0:
        for i in range(1,len(path_indices)):
            weights = reweight(weights, path_indices[i], 2)
    elif outcome == 1:
        weights = reweight(weights, path_indices[target_no], 1/4)
    elif outcome == 2:
        weights = reweight(weights, path_indices[target_no], 1/3)
...
while weight_range > 2:
    weights_norm = weights_norm**0.5
    
    weight_range = np.amax(weights_norm) - np.min(weights_norm[np.nonzero(weights)])
    
plt.imshow(weights_norm.T,interpolation='gaussian', origin='lower')
